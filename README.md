...
A supervisor for Claude Code that learns from your sessions, in Rust. It
wraps any claude launch in a transparent PTY, listens to the session through
hooks, and quietly builds a memory: what you told it about yourself, your
projects, and the people you work with comes back as context in future
...
agent -- --model opus                           # forward args to claude
agent --claude-cmd "ax run --account work --"   # wrap another launcher
...
`--claude-cmd` is split on whitespace — no shell quoting. Wrap anything
fancier in a script.
