...
fn extract_user_settings(claude_args: &[String]) -> (Vec<String>, Option<PathBuf>) {
    ...
    while let Some(argument) = arguments.next() {
        if argument == "--settings" {
            settings = arguments.next().map(PathBuf::from);
...
mod tests {
    ...
    fn user_settings_are_extracted_in_both_flag_forms() {
        ...
        let (remaining, settings) = extract_user_settings(&["--settings=/tmp/mine.json".into()]);
